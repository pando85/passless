...
7b. For fully autonomous browser automation (no native credential selection dialog), use the CDP virtual authenticator workflow described below. This requires extracting the credential's private key from the pass vault and injecting it via `WebAuthn.addCredential`.
...
## Fully autonomous authentication (CDP virtual authenticator)

Chromium shows a native credential selection dialog for discoverable credentials that blocks automation. To achieve fully unattended WebAuthn (no notification, no browser dialog), use CDP's WebAuthn domain with a virtual authenticator injected with the real credential's private key.
...
The virtual authenticator approach extracts the raw private key from the pass vault and injects it into Chromium's virtual authenticator environment. This bypasses the passless daemon's UHID transport and ceremony handler. The key material exists in the CDP client's memory during the session. Use this only in trusted automation environments where the CDP client process is under operator control.
...
- When using CDP virtual authenticators, the raw private key exists in the CDP client process memory. Ensure the client process runs in a trusted environment under operator control.
