...
use soft_fido2::{
    ...
    CredentialRef, CtapCommand, PinStorageCallbacks, Result, UpResult, UvResult,
};
...
impl<S: CredentialStorage> AuthenticatorCallbacks for PasslessCallbacks<S> {
    ...
    fn request_uv(&self, info: &str, user: Option<&str>, rp: &str) -> Result<UvResult> {
        ...
        if storage.disable_user_verification() {
            debug!("User verification handled by backend (e.g., GPG): {}", info);
            return Ok(UvResult::Accepted);
        }
        ...
        match show_verification_notification(
            ...
            Ok(crate::notification::NotificationResult::Accepted) => {
                info!("User verification confirmed via notification");
                Ok(UvResult::Accepted)
            }
            Ok(crate::notification::NotificationResult::Denied) => {
                info!("User verification denied via notification");
                Ok(UvResult::Denied)
            }
            Err(e) => {
                error!("Failed to show UV notification: {}", e);
                Err(soft_fido2::Error::Other)
            }
        }
    }
...
fn build_authenticator_config(security_config: &SecurityConfig) -> AuthenticatorConfig {
...
impl<S: CredentialStorage + 'static> AuthenticatorService<S> {
    pub fn new(storage: S, security_config: SecurityConfig) -> Result<Self> {
        ...
        let callbacks = PasslessCallbacks::new(storage.clone(), security_config.clone());
        let config = build_authenticator_config(&security_config);
        ...
        P: PinStorageCallbacks + Send + Sync + 'static,
    {
        ...
        let config = build_authenticator_config(&security_config);
        let authenticator =
            Authenticator::with_config_and_pin_storage(callbacks, config, pin_storage)?;
