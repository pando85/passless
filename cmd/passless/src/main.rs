...
use pin_storage::{LocalPinStorage, PassPinStorage};
#[cfg(feature = "tpm")]
...
fn run() -> Result<()> {
    ...
    match config.backend().map_err(|e| {
        ...
        BackendConfig::Local { path } => {
            ...
            let service = AuthenticatorService::with_pin_storage(storage, security_config, pin_storage)?;
        ...
        BackendConfig::Pass {
            ...
            let storage = PassStorageAdapter::new(store_path.clone().into(), path.clone().into(), gpg_backend)?;
            let pin_storage = PassPinStorage::new(store_path.into(), path.into())
                .map_err(|e| Error::Other(format!("Failed to create PIN storage: {}", e)))?;
            let service = AuthenticatorService::with_pin_storage(storage, security_config, pin_storage)?;
            run_with_service(service, uhid, shutdown)
        }
        #[cfg(feature = "tpm")]
